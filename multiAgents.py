...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        "*** YOUR CODE HERE ***"
        ghostDistance = [util.manhattanDistance(newPos, ghost) for ghost in successorGameState.getGhostPositions()]
        foodDist = [util.manhattanDistance(newPos, ghost) for food in newFood.asList()]
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        depth = 0
        currentAgentIndex = 0
        val = self.value(gameState, currentAgentIndex, depth)
        return val[0]

    def value(self, gameState, currentAgentIndex, depth):
        currentAgentIndex %= gameState.getNumAgents()
        if depth == self.depth:
            return self.evaluationFunction(gameState)
        if currentAgentIndex == 0:
            return self.maxValue(gameState, currentAgentIndex, depth+1)
        return self.minValue(gameState, currentAgentIndex, depth+1)
